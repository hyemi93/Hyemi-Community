...
<head>
<meta charset="UTF-8">
<title>BoardList(일반게시판_골드)</title>
</head>
<body>
<article>
<h2><%=category%></h2>
...
%>
    <tr class="table-active"> 
      <th scope="row">공지</th>
      <td style="text-align: left;"><a href="BoardView_Level1.jsp?bnum=<%=ndto.getBnum()%>&category=공지사항<%=paging%>"><%=ndto.getTitle()%></a></td>
...
%>
    <tr>
      ...
      <td style="text-align: left;"><div style="padding-left: <%=dto.getDepth()*15%>px; color:#808080;"><%if(dto.getDepth()!=0){ %>└ <%}%>
      								<a href="BoardView_Level3.jsp?bnum=<%=dto.getBnum()%>&category=<%=category%><%=paging%>">
      								<%=dto.getTitle()%></div></a>
      </td>
...
<ul class="pagination">
  <li class="page-item">
  	<!-- 처음 페이지 -->
    <a class="page-link" style="color: #ea868f;" href="BoardList_Level3.jsp?category=<%=category%><%=paging%>" aria-label="Previous">
...
for(pagenum=1; pagenum<=totalcnt2/10+0.9; pagenum++) {%>
	<li class="page-item"><a class="page-link" style="color: #ea868f;" href="BoardList_Level3.jsp?category=<%=category%>&startpage=<%=startpage %>&endpage=<%=endpage%>"><%=pagenum %></a></li>	
<%startpage += 10;
  endpage += 10;
} %>
    ...
    <li class="page-item">
    <!-- 마지막 페이지 -->
      <a class="page-link" style="color: #ea868f;" href="BoardList_Level3.jsp?category=<%=category%>&startpage=<%=startpage-10%>&endpage=<%=endpage-10%>" aria-label="Next">
...
<a href="BoardWrite_Level3.jsp?category=<%=category%>"><input type="button" value="글쓰기" class="btn btn-danger" style="float: right;"></a>
