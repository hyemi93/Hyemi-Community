...
<head>
<meta charset="UTF-8">
<title>BoardView(일반게시판 뷰_브론즈)</title>
</head>
<body>
<article>
	<ul class="list-group">
		...
		<%if(ssaid != null || ssid.equals(dto.getId())){ %>
			<a href="BoardProc.jsp?action=BoardDelete&bnum=<%=bnum%>&category=<%=category%>" onclick="return confirm('글을 삭제하시겠습니까?');">삭제</a>
			<a href="BoardUpdate_Level1.jsp?bnum=<%=bnum%>&category=<%=category%>">수정</a>
		<%} %>
			<Br>
			<hr>
			<%=dto.getContent() %>
...
<ul class="pagination pagination-sm">
  <li class="page-item">
  	<!-- 처음 페이지 -->
    <a class="page-link" style="color: #ea868f;" href="BoardView_Level1.jsp?bnum=<%=bnum%>&category=<%=category%><%=paging%>" aria-label="Previous">
...
for(commnum=1; commnum<=totalcommcnt2/10+0.9; commnum++) {%>
	<li class="page-item"><a class="page-link" style="color: #ea868f;" href="BoardView_Level1.jsp?bnum=<%=bnum%>&category=<%=category%>&startpage=<%=startpage %>&endpage=<%=endpage%>"><%=commnum %></a></li>	
<%startpage += 10;
  endpage += 10;
} %>
    ...
    <li class="page-item">
    <!-- 마지막 페이지 -->
      <a class="page-link" style="color: #ea868f;" href="BoardView_Level1.jsp?bnum=<%=bnum%>&category=<%=category%>&startpage=<%=startpage-10%>&endpage=<%=endpage-10%>" aria-label="Next">
...
<%} %>
	...
	<%} else{ %>
	<a href="BoardReply_Level1.jsp?bnum=<%=bnum%>&category=<%=category%>&depth=<%=depth%>&location=<%=location%>">
		<input type="button" value="답글쓰기" class="btn btn-danger">
	</a>
	<%} %>
	<input type="button" value="목록" class="btn btn-light" onclick="location.href='BoardList_Level1.jsp?category=<%=category%><%=paging%>'">
	</div>
</article>
</body>
