...
create table comment_tbl(
	...
	constraint comment_tbl_fk foreign key(bnum, category)
	references board_tbl(bnum, category) on delete cascade  --부모글 삭제되면 댓글도 삭제
);
...
/*공지글 뷰*/
create or replace view notice_tbl
as
select *
from (select bnum, rebnum, id, nickname, title, content, to_char(bdate, 'yyyy-mm-dd') as bdate, 
      hit, depth, location, commnum, category from board_tbl where category='공지사항'
      start with rebnum is null connect by prior bnum=rebnum order siblings by location asc)
where rownum <=2;
...
/*회원 작성글 뷰*/
create or replace view userboard_tbl
as
...
/*메인 최신글 뷰*/
create or replace view new_tbl
as
select *
from (select bnum, nickname, title, to_char(bdate, 'yyyy-mm-dd') as bdate, bdate as bdate2, hit, commnum, category
      from board_tbl order by bdate2 desc)
where rownum <=5;
